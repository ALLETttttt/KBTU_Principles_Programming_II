...
def main():
    ...
    while True:
        ...
        # For square
            ...
            if Down and PREVX != -1 and PREVY != -1 and CURX != -1 and CURY != -1:
                display.blit(LAYER, (0, 0))
                R = Get_square(PREVX, PREVY, CURX, CURY)
                pygame.draw.rect(display, color, pygame.Rect(R), 1)
                # print("{} {} {} {}".format(prevX, prevY, currentX, currentY))
        ...
        # For right triangle
            ...
            if Down and PREVX != -1 and PREVY != -1 and CURX != -1 and CURY != -1:
                display.blit(LAYER, (0, 0))
                R = right_triangle(PREVX, CURX, PREVY, CURY)
                pygame.draw.polygon(display, color, R, 1)
                # print("{} {} {} {}".format(prevX, prevY, currentX, currentY))
...
def Get_square(x1, y1, x2, y2):
    return pygame.Rect(min(x1, x2), min(y1, y2), min(x1, y2), min(y1, x2))


def right_triangle(x1, x2, y1, y2):
    return [[abs(x1-x2), y2], [x1, abs(y1-y2)], []]
